...
export class User {
    ...
    if (foundUser) {
      ...
      try {
        ...
        if (foundUser) {
          return {
            exists: true,
            err_message: "This userTag has been taken already",
          };
        } else {
          const user = new UserModel({
            name: userObject.name,
            userProfile:
              userObject.userProfile != null
                ? {
          ...
          const payload: ITokenPayload = {
            ...
            name: user.name,
            avatar:
              user.userProfile.avatar != null ? user.userProfile.avatar : null,
            fcm_token: user.fcm_token,
            // userProfile: user.userProfile,
          };

          return {
            token: Utility.createToken(payload),
            user: {
              userTag: user.userTag,
              userID: user.id,
    ...
    try {
      ...
      if (isFollowing === null || undefined) {
        ...
        const follower = await UserModel.findByIdAndUpdate(
          { _id: userID },
          { $inc: { "userProfile.followings": 1 } }
        ).exec();
        const user = await UserModel.findByIdAndUpdate(
          { _id: targetUserID },
          { $inc: { "userProfile.followers": 1 } }
        ).exec();
  ...
  public static async GetUser(
    ...
    try {
      switch (searchKey) {
        ...
        case "followings":
          const followings = await FollowingsModel.find({
            follower: userID,
            target: targetID,
          })
        ...
        case "self":
          const self = await UserModel.findById(userID, { password: 0 })
            .lean()
            .exec();
        ...
        case "user":
          const user = await UserModel.findById(targetID, { password: 0 })
            .lean()
            .exec();
  ...
  public static async UploadAvatar(file: any, userID: string) {
    try {
      ...
      const user = await UserModel.findByIdAndUpdate(userID, {
        "userProfile.avatar": data.Location,
      }).exec();
  ...
  public static async GetUserPosts(
    ...
    try {
      const posts = await AggregationQueries.GetUserPostsAggreg(userID, {
        page,
        limit,
      });
  ...
  public static async AvailableUserTag(userTag: string) {
    const available = await UserModel.findOne({
  ...
  public static async ConnectUser(
    ...
    if (filter === "sameCampus") {
      connectUsers = await UserModel.find(
      ...
      connectUsers = await UserModel.find(
        { "userProfile.university": { $ne: campus } },
        { name: 1, userProfile: 1, userTag: 1 }
      )
        .lean()
        .exec();
  ...
  public static async GetUserWithEmail(email: string) {
    ...
    if (foundUser) {
      ...
      return { exists: false, err_message: "This user doesn't exist" };
    }
  }
  public static async saveOTP(otp: string, email: string) {
    const user = await UserModel.findOneAndUpdate(
      { email: email },
      { $set: { otp: otp } },
    ...
    if (user) {
      console.log(user);
      return { save: true, emailaddress: user.email };
    } else {
      return { save: false };
    }
  }
